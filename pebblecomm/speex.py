...
def store_data(frames, filename):
    frame_sz = 160
    rate = 8000
    version = "1.2rc1"
...
def test_crc():
    spx = create_speex_header("1.2rc1", 8000, 160)
    ogg = create_ogg_packet(True, False, 0, 0x42E296FC, 0, [spx])

    crc = struct.unpack('I',ogg[22:26])[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('tty', type=str, nargs='?',
                        help="Serial port (e.g. /dev/cu.usbserial-xxxxxxxB or /dev/ttyUSB0)")
    parser.add_argument('--output', type=str,
                        help="Output file name (default: 'build/test.ogg'")
    parser.add_argument('--decode', type=str,
                        help="Decode to specified file. No encoding done if not specified")
    args = parser.parse_args()

    tty = args.tty #or _guess_tty()
    if tty:
        serial_port = SerialPortWrapper(tty)
        try:
            spx_frames = receive_hdlc_data(serial_port)
        finally:
            serial_port.close()
        f = store_data(spx_frames, args.output or 'build/test.ogg')
        if args.decode:
            decode(f, args.decode)
    else:
